      ...
      >
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <Input label="Username" placeholder="coolcoder42"
              value={form.username}
              onChange={(e) => setForm({ ...form, username: e.target.value })}
              error={errors.username} icon={<User size={16}/>}
            />
          ...
          <Button fullWidth size="lg" onClick={handleSubmit} loading={loading} style={{ marginTop: '24px' }}>
            Create account
          </Button>

          <p style={{ textAlign: 'center', marginTop: '20px', color: '#8888aa', fontSize: '14px' }}>
            Already have an account?{' '}
            <Link href="/login" style={{ color: '#8b5cf6', fontWeight: 500 }}>Log in</Link>
          </p>
        </div>
      </motion.div>
    </div>
