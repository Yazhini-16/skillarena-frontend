...
import { Wallet, ArrowDownLeft, ArrowUpRight, Plus, Clock, Shield } from 'lucide-react';
...
export default function WalletPage() {
  ...
  const { balance, lockedBalance, transactions,
          setWallet, setTransactions, updateBalance }      = useWalletStore();
  ...
  const handleRazorpayPayment = useCallback(async () => {
    ...
    if (amount > 10000) { toast.error('Maximum deposit is ₹10,000'); return; }
    ...
    try {
      ...
      const options = {
        ...
        handler: async (response) => {
          // Step 3: Verify payment on backend — wallet is credited here
          try {
            ...
            const { balance: newBalance, amount: creditedAmount } = verifyRes.data.data;
            updateBalance(newBalance);
            setDepositAmount('');
            toast.success(`₹${creditedAmount} added to your wallet!`);
            loadWallet(); // Refresh transaction history
  ...
  return (
    <div style={{ minHeight: '100vh', background: '#0a0a0f' }}>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '40px 24px' }}>
        ...
        <h1 style={{ fontSize: '32px', fontWeight: 700, marginBottom: '32px' }}>Wallet</h1>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '16px', marginBottom: '32px' }}>
          ...
          <motion.div
            ...
            style={{
              ...
              border: '1px solid #2a2a3a', borderRadius: '16px', padding: '28px',
            }}
          >
            <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '12px', color: '#8888aa', fontSize: '13px' }}>
              <Wallet size={14}/>
              Available balance
            </div>
            <div style={{ fontSize: '44px', fontWeight: 800, color: '#f0f0f5', letterSpacing: '-0.02em' }}>
              ₹{parseFloat(balance).toFixed(2)}
            </div>
            {lockedBalance > 0 && (
              <div style={{
                fontSize: '13px', color: '#f59e0b', marginTop: '10px',
          ...
          >
            <div style={{ fontSize: '13px', color: '#8888aa', marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '6px' }}>
              <Plus size={14}/>
              Add funds
            </div>
              ...
              {QUICK_AMOUNTS.map(amt => (
                <button
                  ...
                  style={{
                    ...
                    background: depositAmount === String(amt) ? 'rgba(124,58,237,0.2)' : '#1a1a24',
                    border: `1px solid ${depositAmount === String(amt) ? '#7c3aed' : '#2a2a3a'}`,
                    color: depositAmount === String(amt) ? '#8b5cf6' : '#8888aa',
            ...
            <div style={{ display: 'flex', gap: '8px', marginBottom: '12px' }}>
              <input
                ...
                onKeyDown={(e) => e.key === 'Enter' && handleRazorpayPayment()}
                style={{
              ...
              />
              <Button onClick={handleRazorpayPayment} loading={loading} disabled={!depositAmount}>
                Pay
              </Button>
            </div>
        ...
        <div>
          ...
          {txLoading ? (
            ...
            <div style={{
              ...
              color: '#55556a', background: '#111118',
                  ...
                  >
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                      ...
                      <div>
                        ...
                        <div style={{ fontSize: '12px', color: '#55556a', marginTop: '2px' }}>
